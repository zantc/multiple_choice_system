...
try {
    $results = $pdo->query("
        SELECT er.id, er.score, er.correct_count AS total_correct, er.total_questions, COALESCE(er.submitted_at, er.started_at, er.completed_at) AS completed_at,
    ...
    $results = $pdo->query("
        SELECT er.id, er.score, er.correct_count AS total_correct, er.total_questions, er.completed_at,
               u.full_name, u.username, u.email,
               e.title as exam_title
        FROM exam_results er
        JOIN users u ON er.user_id = u.id
...
$pageTitle = "Chấm điểm & Kết quả thi | Portal Giáo viên";
$hideNavbar = true;
...
<div class="main-content">
    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid var(--line); padding-bottom: 15px; margin-bottom: 20px;">
        <h1 style="margin: 0; color: var(--sidebar-bg); font-size: 24px;">
            <i class="fa-solid fa-marker" style="color: var(--primary);"></i> Chấm điểm & Kết quả thi
        </h1>
        <a href="teacher_sessions.php" class="btn btn-primary" style="font-size: 13px;">
            <i class="fa-solid fa-calendar-check"></i> Quản lý Ca thi
        </a>
    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="actions">
                                ...
                                <form method="POST" style="display:inline; margin:0;" onsubmit="return confirm('Bạn muốn gửi điểm này về email của học sinh?')">
                                    <?= csrf_field() ?>
...
<div id="editScoreModal" class="modal">
    <div class="modal-content" style="max-width: 450px;">
        ...
        <form method="POST" id="scoreForm">
            ...
            <input type="text" id="edit_student_name" disabled style="background: #eee; padding: 11px;">
